...
function getAnimeInfo(episode, animeName) {
  return new Promise((resolve) => {
    chrome.runtime.sendMessage(
      { action: "processInfo", episode, animeName },
      (response) => {
        resolve(response);
      }
    );
...
function setTitle(titleElement, category, color,side) {
  ...
  spanElement.style.marginInline = "10px";
  ...
  if (side == POSITION.LEFT) {
      ...
      titleElement.parentNode.appendChild(spanElement);
  }
}
...
function getSide(siteElementsID, site) {
    return siteElementsID[site]?.side || null;
}
...
function getPlaceSpanID(siteElementsID, site) {
    return siteElementsID[site]?.placeSpan || null;
}
function getNameAndEpisode(titleComponent, subTitleComponent, site) {
  ...
  switch (site) {
    ...
    case "9animetv.to":
      animeEpisode =
        titleComponent.querySelector("#cm-episode-number").textContent;
      animeName = subTitleComponent.textContent;
      break;
...
async function main(siteElementsID) {
  ...
  const placeSpanId = getPlaceSpanID(siteElementsID, site);

  const titleComponent = await getElementInDOM(titleID);
  const subTitleComponent = await getElementInDOM(subTitleID);
  const placeSpan = await getElementInDOM(placeSpanId);
  const side = getSide(siteElementsID, site);
  const { animeEpisode, animeName } = getNameAndEpisode(
    titleComponent,
    subTitleComponent,
    site
  );
  ...
  const { category: nextCategory } = await getAnimeInfo(
    animeEpisode + 1,
    animeName
  );
  const { category, color, imgURL } = await getAnimeInfo(
    animeEpisode,
    animeName
  );
  ...
  if (category && color) {
    setTitle(placeSpan, category.category, color,side);
  }
  ...
  try {
    const siteElementsID = {
      "www.crunchyroll.com": {
        ...
        nextEpisodeID: "a.playable-card-mini-static__link--UOJQm",
        placeSpan: "h1",
        side: POSITION.LEFT,
      },
      "www3.animeflv.net": {
        ...
        nextEpisodeID: "a.CapNvNx.fa-chevron-right",
        placeSpan: "h1.Title",
        side: POSITION.LEFT,
      },
      "9animetv.to": {
        titleID: 'a.btn.btn-sm.btn-comment-tab[data-type="episode"]',
        subTitleID: "h2.film-name",
        nextEpisodeID: "a.btn.btn-sm.btn-next",
        placeSpan: "li.breadcrumb-item:last-child",
        side: POSITION.RIGHT,
      },
      "animevision.masterstreet.cl": {
        titleID: "h1.lang-name",
        subTitleID: "h4.episodes-display",
        nextEpisodeID: "button.next-button",
        placeSpan: "h1.lang-name",
        side: POSITION.LEFT,
      },
    };
